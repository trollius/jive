...
make.jive <- function(simmap, traits, model_var="OU1", model_mean="BM", model_lik="MVN"){
	...
	if (name.check(simmap, traits) != "OK") {
		...
		if (model_mean == "BM") {
			...
			jive$prior_mean$hpriorUnif		<- hpriorUnif
			jive$prior_mean$hpriorGamma		<- hpriorGamma
		...
		if (model_var == "OU1") {
			...
			jive$prior_var$hprioUnif		<- hpriorUnif
			jive$prior_var$hpriorGamma		<- hpriorGamma
...
##--------------------------- define hyperprior functions
# uniform hyperprior, min-max bounded
hpriorUnif <- function(x){

	hp <- dunif(mmvn, min=-10000, max=10000, log=TRUE)
	return(hp)
	
}

# gamma hyperprior
hpriorGamma<- function(x){

	hp <- dgamma(smvn, shape=1.1, scale=5, log=TRUE)
	return(hp)
}
