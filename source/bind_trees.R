##' Function to simulate species trees with complex diversification scenarios, 
##' with changing diversification rates at inner nodes of the phylogeny 
##' and mass extinction events at particular random times in the past.
##' 
##' 
##'
...
bindTwoTrees <- function(acceptor, donnor, nb.spec){#binds two different trees and keeps them ultrametric
  donnor.stem.age <- branching.times(donnor)[1] + donnor$root.edge # total age of the tree
  br.ti <- branching.times(acceptor)
  labels <- c(acceptor$tip.label, acceptor$node.label)
  if (br.ti[1] < donnor.stem.age) samp <- acceptor$node.label[1] # if the total age of the donor tree is larger than the time of mrca of the acceptor tree, then we bind both trees at their root
    ...
    samp.vect <- edges[br.ti[edges[, 2]]<donnor.stem.age & br.ti[edges[, 1]]>donnor.stem.age, 2]
    samp <- sample(samp.vect, 1)
  }
  k <- (which(labels==samp))
  new.tree <- bind.tree(acceptor, donnor, where = k, position = donnor.stem.age - br.ti[samp])#we bind it at the right position (branching time of the edge selected minus age of the donor tree) so that we have a ultrametric tree
  new.tree$node.label[is.na(new.tree$node.label)] <- paste("n", nb.spec, sep="") #we rename the node label that has been erased
  names(new.tree$edge.length) <- NULL
  if (is.ultrametric(new.tree)) return(new.tree)
...
sorting_and_binding_all_trees <- function(list_of_trees, param){
  ...
  for (i in 1:length(list_of_trees)) {#we define the root edge as the difference between the total age of the tree and the time of mrca
    ...
    nb.spec <- nb.spec+Ntip(list_of_trees[[i]][[1]])
  }
  ...
  for (i in 1:(length(ordered)-1)){# we begin by the most ancient and sequentially bind the young trees to it
    final_tree <- bindTwoTrees(final_tree, list_of_trees[[as.numeric(names(ordered)[i+1])]][[1]], nb.spec+i+1)
  }
